...
def main(args: Arguments, logger: logging.Logger):
    secret_type = 'prompt'
    out_file = os.path.join(args.train.output_dir, f"generated_{secret_type}_random.json")
    ...
    for text in texts:
        ...
        output = model.generate(input_ids, **gen_kwargs_random)
        # decoded_output = tokenizer.decode(output[0].tolist(), skip_special_tokens=True)
        output_text = tokenizer.batch_decode(output, skip_special_tokens=True)
        completion = [ot.replace(text, '') for ot in output_text]
