...
class PrivacyArguments:
    ...
    delta_exponent: float = field(default=1.0, metadata={
        "help": "Exponent for calculating target delta, defaults to 1.0"
    })
    ...
    accountant: str = field(default='prv', metadata={
        "help": "DP accountant to use. Options: prv, rdp, ctd."
    })

    def initialize(self, sampling_probability: float, num_steps: int, num_samples: int) -> None:
        print('------------------------------------')
        print('initializing privacy arguments')
        print(f'sampling_probability: {sampling_probability:.4e}')
        print(f'num_steps: {num_steps}')
        print(f'num_samples: {num_samples}')

        if self.target_delta is None:
            self.target_delta = 1.0/(num_samples**self.delta_exponent)
        logger.info(f"The target delta is set to be: {self.target_delta}")

        # Set up noise multiplier
        if self.noise_multiplier is None:
            if self.accountant == 'prv':
                print('Using prv accountant')
                eps_error = 0.01
                while True:
                    try:
                        ...
                        print(f"Function succeeded with eps_error = {eps_error}")
                        self.eps_error = eps_error
                        break  # Exit the loop if successful
                    except Exception as e:
                        print(f"Error occurred with eps_error = {eps_error}: {e}")
                        eps_error += 0.01  # Increment eps_error
                        if np.isclose(eps_error, 0.1):
                            print('error increased to 0.1 -- switch to using connect-the-dots accountant')
                            self.noise_multiplier = calculate_sigma_sgd(
                                epsilon=self.target_epsilon,
                                delta=self.target_delta,
                                ratio=sampling_probability,
                                iters=num_steps
                            )
                            self.eps_error = 0.01
                            break
                ...
                print('Using connect-the-dots accountant')
                self.noise_multiplier = calculate_sigma_sgd(
                    epsilon=self.target_epsilon,
                    delta=self.target_delta,
                    ratio=sampling_probability,
        ...
        print(f'target epsilon: {self.target_epsilon:.4e}')
        print(f'target delta: {self.target_delta:.4e}')
        print(f'The noise multiplier is set to be: {self.noise_multiplier:.4e}')
        print('------------------------------------')
