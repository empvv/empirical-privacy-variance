...
class DPCallback(TrainerCallback):
    ...
    def __init__(
        ...
        if prv_accountant == rdp_accountant:
            self.compute_prv_epsilon = lambda s: self.rdp_accountant.get_epsilon(self.target_delta)
        else:
...
class GradSampleModule(opacus.GradSampleModule):
    ...
    def __getattr__(self, name):
        # Redirect attribute access to self._module if the attribute isn't found
        if name in ['_module', 'no_sync']:
        ...
        **kwargs: Dict
        ...
        if not self.privacy_args.is_initialized:
            self.privacy_args.initialize(
                ...
                num_steps=self.num_steps if not args.max_steps else args.max_steps,
        ...
        try:
            self.prv_accountant = PRVAccountant(
                ...
                eps_error=self.privacy_args.eps_error,
                max_compositions=self.num_steps if not args.max_steps else args.max_steps,
            )
            ...
            self.get_rdp_epsilon = lambda: self.rdp_accountant.get_epsilon(self.privacy_args.target_delta)  # RDP epsilon
            self.get_prv_epsilon = lambda: self.rdp_accountant.get_epsilon(self.privacy_args.target_delta)  # RDP epsilon
            self.dp_callback = DPCallback(
                noise_multiplier=self.privacy_args.noise_multiplier,
                target_delta=self.privacy_args.target_delta,
                sampling_probability=self.sampling_probability,
                rdp_accountant=self.rdp_accountant,
                prv_accountant=self.rdp_accountant
            )
