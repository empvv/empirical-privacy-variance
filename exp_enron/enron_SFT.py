...
class DataArguments:
    ...
    prompt_len: int = field(default=10, metadata={
        "help": "Prompt length"
    })
...
def build_dataset(args, tokenizer, train_ratio, dataset_name="imdb", input_min_text_length=2, input_max_text_length=8):
    ...
    ds = datasets.load_dataset(
        'csv', 
        data_files={
            'train': f'enron_data/{dataset_name}/train.csv', 
            'val': f'enron_data/{dataset_name}/valid.csv', 
            'test': f'enron_data/{dataset_name}/test.csv'}
    )
    
    print(dataset_name)
    print(ds)
    
    if args.data.split != 'test':
    ...
    with args.train.main_process_first(desc="tokenizing dataset"):
        if args.data.split != 'test':
            ...
            ds_test = ds_test.map(
                preprocess_function, batched=True, desc="tokenizing dataset", remove_columns=ds_test.column_names
            )
            return None, ds_test
